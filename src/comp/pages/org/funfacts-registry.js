...
import { createFunfact, fetchFunfacts, deleteFunfacts, fetchModules } from '../../../redux';
import { funfactColumns, GlobalFilter, Checkbox } from '../../tables';
import { CKEditor } from '@ckeditor/ckeditor5-react';
import ClassicEditor from '@ckeditor/ckeditor5-build-classic';
import ContentHeader from '../../master/contentHeader';

function FunFactRegistry({
    createFunfact, createFunfactData, fetchFunfacts, fetchFunfactsData, deleteFunfacts, 
    deleteFunfactsData, fetchModules, fetchModulesData
  }) {
  ...
  useEffect(() => {
    if (deletion) {
      ...
      selectedRows.forEach(funfact => funfacts.push({ parameter: funfact.uuid }));
      deleteFunfacts({ funfacts });
      setDeletion(false);
      setReload(true);
    }
  }, [deletion]);
  ...
  }, [createFunfactData.createFunfact, deleteFunfactsData.deleteFunfacts]);

  const columns = useMemo(() => funfactColumns, []);
  const data = useMemo(() => fetchFunfactsData.fetchFunfacts, [ fetchFunfactsData.fetchFunfacts ]);
  const {
  ...
  return (
  <>
    <ContentHeader headerData={{ title: "Fun Facts", subTitle: "New & Registry", pageName: "Fun Facts / New & Registry" }}/>
          ...
          <div className="nav-tabs-custom">
            ...
            <div className="tab-content">
              ...
              { deleteFunfactsData.loading ?
                <><img className="spinner-20" src="dist/img/spinner/spinner-2.gif" alt=""/> Deleting fun facts...</>
              : deleteFunfactsData.error ?
                <pre>{JSON.stringify(deleteFunfactsData.error, null, 2)}</pre>
              : deleteFunfactsData.deleteFunfacts && Object.entries(deleteFunfactsData.deleteFunfacts).length > 0 ?
                <h4 className="text-success text-center">Success! Deleted fun facts</h4>
              : null }

              { fetchFunfactsData.loading ?
                <><img className="spinner-20" src="dist/img/spinner/spinner-2.gif" alt=""/> Loading fun facts...</>
              : fetchFunfactsData.error ?
                <pre>{JSON.stringify(fetchFunfactsData.error, null, 2)}</pre>
              : fetchFunfactsData.fetchFunfacts ?
                <>
              ...
              { createFunfactData.loading ?
                <><img className="spinner-20" src="dist/img/spinner/spinner-2.gif" alt=""/> Saving fun fact...</>
              : createFunfactData.error ?
                <pre>{JSON.stringify(createFunfactData.error, null, 2)}</pre>
              : createFunfactData.createFunfact && Object.entries(createFunfactData.createFunfact).length > 0 ?
                <h4 className="text-success text-center">Success! Fun fact created</h4>
              : null }
...
const mapStateToProps = state => {
  return {
    createFunfactData: state.createFunfactData,
    fetchFunfactsData: state.fetchFunfactsData,
...
const mapDispatchToProps = dispatch => {
  return {
    createFunfact: funfact => dispatch(createFunfact(funfact)),
    fetchFunfacts: () => dispatch(fetchFunfacts()),
    deleteFunfacts: funfacts => dispatch(deleteFunfacts(funfacts)),
    fetchModules: () => dispatch(fetchModules()),
  }
}

export default connect(mapStateToProps, mapDispatchToProps)(FunFactRegistry);