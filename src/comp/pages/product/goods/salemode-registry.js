...
import {
  createSalemode, fetchSalemodes, deleteSalemodes, fetchModules, fetchDistributions
} from '../../../../redux';
import { salemodeColumns, GlobalFilter, Checkbox } from '../../../tables';
import { CKEditor } from '@ckeditor/ckeditor5-react';
import ClassicEditor from '@ckeditor/ckeditor5-build-classic';
import ContentHeader from '../../../master/contentHeader';

function SalemodeRegistry({
    createSalemode, salemodeCreateData, fetchSalemodes, salemodesFetchData, deleteSalemodes, 
    salemodesDeleteData, fetchModules, fetchModulesData, fetchDistributions, distributionsFetchData
  }) {
  ...
  useEffect(() => {
    if (deletion) {
      ...
      selectedRows.forEach(salemode => salemodes.push({ parameter: salemode.uuid }));
      deleteSalemodes({ salemodes });
      setDeletion(false);
      setReload(true);
    }
  }, [deletion]);
  ...
  }, [salemodeCreateData.salemodeCreate, salemodesDeleteData.salemodesDelete]);

  const columns = useMemo(() => salemodeColumns, []);
  const data = useMemo(() => salemodesFetchData.salemodesFetch, [ salemodesFetchData.salemodesFetch ]);
  const {
  ...
  return (
  <>
    <ContentHeader headerData={{ title: "Sale Mode", subTitle: "New & Registry", pageName: "Sale Mode / New & Registry" }}/>
          ...
          <div className="nav-tabs-custom">
            ...
            <div className="tab-content">
              ...
              { salemodesDeleteData.loading ?
                <><img className="spinner-20" src="dist/img/spinner/spinner-2.gif" alt=""/> Deleting salemodes...</>
              : salemodesDeleteData.error ?
                <pre>{JSON.stringify(salemodesDeleteData.error, null, 2)}</pre>
              : salemodesDeleteData.salemodesDelete && Object.entries(salemodesDeleteData.salemodesDelete).length > 0 ?
                <h4 className="text-success text-center">Success! Deleted salemodes</h4>
              : null }

              { salemodesFetchData.loading ?
                <><img className="spinner-20" src="dist/img/spinner/spinner-2.gif" alt=""/> Loading salemodes...</>
              : salemodesFetchData.error ?
                <pre>{JSON.stringify(salemodesFetchData.error, null, 2)}</pre>
              ...
              { salemodeCreateData.loading ?
                <><img className="spinner-20" src="dist/img/spinner/spinner-2.gif" alt=""/> Saving salemode...</>
              : salemodeCreateData.error ?
                <pre>{JSON.stringify(salemodeCreateData.error, null, 2)}</pre>
              : salemodeCreateData.salemodeCreate && Object.entries(salemodeCreateData.salemodeCreate).length > 0 ?
                <h4 className="text-success text-center">Success! Salemode created</h4>
              : null }
...
const mapStateToProps = state => {
  return {
    salemodeCreateData: state.salemodeCreateData,
...
const mapDispatchToProps = dispatch => {
  return {
    createSalemode: distribution => dispatch(createSalemode(distribution)),
    fetchSalemodes: () => dispatch(fetchSalemodes()),
    deleteSalemodes: distributions => dispatch(deleteSalemodes(distributions)),
    fetchModules: () => dispatch(fetchModules()),
    fetchDistributions: () => dispatch(fetchDistributions()),
  }
}

export default connect(mapStateToProps, mapDispatchToProps)(SalemodeRegistry);